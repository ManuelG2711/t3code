...
import { makePiAgentTextGeneration } from "../../textGeneration/PiAgentTextGeneration.ts";
    ...
    Effect.gen(function* () {
      ...
      const snapshot = yield* makeManagedServerProvider<PiAgentSettings>({
        ...
        initialSnapshot: (settings) =>
          buildInitialPiAgentProviderSnapshot(settings).pipe(Effect.map(stampIdentity)),
        checkProvider,
