...
import { PiAgentDriver } from "../Drivers/PiAgentDriver.ts";
...
const makePiAgentConfig = (overrides: Partial<PiAgentSettings>): PiAgentSettings => ({
  enabled: false,
  binaryPath: "pi-acp",
  launchArgs: "",
  customModels: [],
...
const normalizeHomeContinuationKey = (value: string | undefined): string | undefined =>
  value?.replaceAll("\\", "/").replace(/^([^:]+):home:[A-Z]:\/home\//u, "$1:home:/home/");
...
describe("ProviderInstanceRegistryLive — multi-instance codex slice", () => {
  ...
  it.live("boots two independent codex instances from a ProviderInstanceConfigMap", () =>
    Effect.gen(function* () {
      ...
      expect(normalizeHomeContinuationKey(personalSnapshot.continuation?.groupKey)).toBe(
      ...
      expect(normalizeHomeContinuationKey(workSnapshot.continuation?.groupKey)).toBe(
...
describe("ProviderInstanceRegistryLive — all drivers slice", () => {
  ...
  it.live("boots one instance of every shipped driver from a single config map", () =>
    Effect.gen(function* () {
      ...
      const piAgentId = ProviderInstanceId.make("piagent_default");
      ...
      const piAgentDriverKind = ProviderDriverKind.make("piAgent");
      ...
      const configMap: ProviderInstanceConfigMap = {
        ...
        [piAgentId]: {
          driver: piAgentDriverKind,
          displayName: "Pi Agent",
          enabled: false,
          config: makePiAgentConfig({ launchArgs: "-y pi-acp" }),
        },
      };
      ...
      expect(instances.map((instance) => instance.instanceId).toSorted()).toEqual(
        [codexId, claudeId, cursorId, grokId, openCodeId, piAgentId].toSorted(),
      );
      ...
      const piAgent = yield* registry.getInstance(piAgentId);
      expect(codex?.driverKind).toBe(codexDriverKind);
      expect(claude?.driverKind).toBe(claudeDriverKind);
      expect(cursor?.driverKind).toBe(cursorDriverKind);
      expect(grok?.driverKind).toBe(grokDriverKind);
      expect(openCode?.driverKind).toBe(openCodeDriverKind);
      expect(piAgent?.driverKind).toBe(piAgentDriverKind);
      expect(codex?.displayName).toBe("Codex");
      expect(claude?.displayName).toBe("Claude");
      expect(cursor?.displayName).toBe("Cursor");
      expect(grok?.displayName).toBe("Grok");
      expect(openCode?.displayName).toBe("OpenCode");
      expect(piAgent?.displayName).toBe("Pi Agent");
      ...
      expect(normalizeHomeContinuationKey(codexSnapshot.continuation?.groupKey)).toBe(
        "codex:home:/home/julius/.codex",
      );
      ...
      expect(normalizeHomeContinuationKey(claudeSnapshot.continuation?.groupKey)).toBe(
      ...
      const piAgentSnapshot = yield* piAgent!.snapshot.getSnapshot;
      expect(piAgentSnapshot.instanceId).toBe(piAgentId);
      expect(piAgentSnapshot.driver).toBe(piAgentDriverKind);
      expect(piAgentSnapshot.enabled).toBe(false);
      expect(piAgentSnapshot.continuation?.groupKey).toBe(
        `${piAgentDriverKind}:instance:${piAgentId}`,
      );
    }).pipe(Effect.provide(testLayer)),
  );
});
